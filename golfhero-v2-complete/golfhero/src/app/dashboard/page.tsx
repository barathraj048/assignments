...
import {
  Trophy, Target, Heart, Calendar, TrendingUp,
  Plus, Edit2, Trash2, ChevronRight, AlertCircle,
  CheckCircle, Clock, CreditCard, ArrowUpRight
} from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import type { GolfScore, Subscription, DrawMonth, DrawResult, CharityContribution } from '@/types'
...
export default function DashboardPage() {
  ...
  async function loadDashboardData() {
    ...
    const [profileRes, scoresRes, subRes, drawRes, resultsRes, contribRes] = await Promise.all([
      ...
      supabase.from('draw_months').select('*').in('status', ['upcoming', 'open']).order('month_year', { ascending: false }).limit(1).single(),
    ...
    if (resultsRes.data) setRecentResults(resultsRes.data as any)
    if (resultsRes.data) {
      setTotalWinnings(resultsRes.data.reduce((sum: number, r: any) => sum + (r.payout_status === 'paid' ? r.prize_amount : 0), 0))
    }
    if (contribRes.data) {
      setCharityTotal(contribRes.data.reduce((sum, c) => sum + c.amount, 0))
    }
    setLoading(false)
  }
          ...
          <div className="flex items-center gap-3">
            {subscription?.charity && (
              <div className="hidden sm:flex items-center gap-2 text-sm text-slate-400">
                ...
                {(subscription.charity as any).name}
              </div>
            )}
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                {
                  label: 'Charity total',
                  value: `£${charityTotal.toFixed(2)}`,
                  sub: subscription?.charity ? (subscription.charity as any).name : 'No charity selected',
                  icon: Heart,
                  color: 'pink',
                },
                {
        ...
        {activeTab === 'charity' && (
          ...
          >
            <div>
              ...
              <p className="text-slate-400 text-sm">Track your contributions and change your chosen cause.</p>
            </div>
            ...
            {subscription?.charity ? (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <div className="font-semibold">{(subscription.charity as any).name}</div>
                    <div className="text-sm text-slate-400">{(subscription.charity as any).short_description}</div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="bg-slate-800/60 rounded-xl p-4">
                    <div className="text-2xl font-bold text-pink-400">£{charityTotal.toFixed(2)}</div>
                    <div className="text-xs text-slate-500 mt-1">Your total contribution</div>
                  </div>
                  <div className="bg-slate-800/60 rounded-xl p-4">
                    <div className="text-2xl font-bold text-emerald-400">{subscription.charity_percentage}%</div>
                    <div className="text-xs text-slate-500 mt-1">Of your subscription</div>
                  </div>
                </div>
                <p className="text-xs text-slate-500">
                  Minimum 10% of your subscription goes to this charity automatically.
                  You can increase this in your account settings.
                </p>
              </div>
            ) : (
              <div className="bg-slate-900 border border-amber-500/30 rounded-2xl p-6 text-center">
                ...
                <p className="text-amber-400 font-medium mb-2">No charity selected</p>
                <p className="text-slate-400 text-sm mb-4">Choose a charity to direct 10% of your subscription to a cause you care about.</p>
                <a href="/charities" className="text-emerald-400 hover:text-emerald-300 text-sm font-medium">
                  Browse charities →
                </a>
              </div>
            )}
            ...
            <div className="bg-gradient-to-br from-emerald-900/30 to-teal-900/20 border border-emerald-500/20 rounded-2xl p-6">
              <h3 className="font-semibold mb-4 text-emerald-300">Platform-wide impact</h3>
              <div className="text-4xl font-black text-white mb-1">£352,400</div>
              <div className="text-sm text-emerald-400/70">raised for 23 charities by our members</div>
            </div>
          </motion.div>
        )}
        ...
        {activeTab === 'draws' && (
          ...
          >
            ...
            {recentResults.some(r => r.verification_status === 'pending' && r.prize_amount > 0) && (
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5">
                ...
                <p className="text-sm text-slate-400 mb-4">
                  Upload a screenshot of your scores from your golf platform to claim your prize.
                </p>
                <button className="bg-amber-500 hover:bg-amber-400 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all">
                  Upload proof
                </button>
              </div>
            )}
